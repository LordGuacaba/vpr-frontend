...
function App() {
  ...
  const getCaptions = (filename) => {
    ...
    axios
      .get(`${baseUrl}/presentation/${filename}`, { responseType: "blob" })
      .then((response) => {
        if (response.status === 200) {
          ...
          axios
            .delete(`${baseUrl}/presentation/${outputName || "expanded"}`)
  ...
  const handleFileSubmission = (e) => {
    ...
    axios
      .post(`${baseUrl}/create?name=${outputName}`, formData, config)
      .catch((error) => console.error("Error: ", error))
